...
>
> **✅ AM 重试配置已锁定**：
> - 作业侧 `application-attempts=0` + 集群侧 `am.max-attempts` 未配置 + `am.global.max-attempts` 未配置
> - 经源码推导（`RMAppImpl.java:457-478`）+ 30 天稳定运行实证 → **实际生效 `maxAppAttempts = 2`**
> - **HA 能工作**，但 AM 只能重试 1 次（较为脆弱），DP 平台侧提供第二层自愈兜底
>
> **🟡 真正的核心风险**：这是 **DP 平台托管**作业（`dp-beaver-master-QF64y0ti.jar`），**必须通过平台操作**，手工 kill 会和平台自愈逻辑冲突（详见 § 2.4）。
>
> **⚠️ NM 关机的真实隐患**（基于 maxAppAttempts=2）：
> - 如果 AM 所在节点关机 → 消耗 1 次重试（Attempt _02）→ 此后容错余量为 0
> - 如果节点上线前再发生一次 AM 失败（1.13 Bug / 资源问题）→ 作业 FAILED → DP 平台自愈介入
...
**关键事实**：
- 作业已**稳定运行 30 天+**
- 集群 yarn-site.xml 中 `yarn.resourcemanager.am.max-attempts` **未配置**
- 集群 yarn-site.xml 中 `yarn.resourcemanager.am.global.max-attempts` **未配置**

**✅ 已可锁定结论**：根据源码推演 + 两个未配置事实 + 30 天运行实证，**实际生效 `maxAppAttempts = 2`**（DEFAULT 值）。
...
#### 3. 🎯 **锁定结论**：`maxAppAttempts = 2`

代入源码公式（`RMAppImpl.java:457-478`）：

```
conf.getInt("yarn.resourcemanager.am.max-attempts", 2)  // 未配置 → 2
        ↓
rmMaxAppAttempts = 2
globalMaxAppAttempts = 2
        ↓
作业侧 individualMaxAppAttempts = 0 → 进入 <=0 分支
        ↓
this.maxAppAttempts = rmMaxAppAttempts = 2
```
...
| 验证角度 | 结论 |
|---|---|
| 源码推导 | `maxAppAttempts = 2` |
| 30 天稳定运行 | `maxAppAttempts >= 1`（否则 AM 启动不起来）✅ 一致 |
| 如果是 0 的假设 | 作业根本活不过第一次提交 ❌ 与事实矛盾 |

**含义**：
- AM **可以失败 1 次并自动重启**，第二次失败就 FAILED
- HA 在 AM 级别**是工作的**，只是重试次数较少（相比生产推荐的 10）
- 作业 30 天稳定 = 这 30 天内 AM 要么没失败，要么失败次数在 1 次以内
